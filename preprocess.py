    ...
    def preprocess(self, path: str):
        ...
        if self.n_rmv_front != 0 or self.n_rmv_back != 0:
            for i in range(int(np.max(pykoop_dict['X_valid'][:, 0])) + 1):
                temp = self.n_rmv_front if i == 2 else 0
                temp_val = pykoop_dict['X_valid'][
                    pykoop_dict['X_valid'][:, 0] == i, :][temp:, :]
        ...
        if self.noise != 0:
            for i in range(int(np.max(pykoop_dict['X_train'][:, 0]))):
                ...
                x_temp, snr, _ = utilities.add_noise(np.block([[x1_temp],
                                                               [x2_temp]]),
                                                     Q,
                                                     0,
                                                     1,
                ...
                # ax.set_xticks(np.linspace(-4, 8, 7))
                # ax.set_aspect('equal')
                # ax.grid(True, linestyle='--')
                # fig.legend(
                #     bbox_to_anchor=(0.75, 0.37),
                #     loc='upper center',
                #     ncol=1,
                # )
                # fig.savefig('build/figures/test.pdf', bbox_inches='tight')
...
class PreprocessNonlinearMassSpringDamper:
    ...
    def preprocess(self, path: str):
        ...
        for tag, n_eps in n_eps.items():
            for ep in range(n_eps):
                ...
                X = np.concatenate((np.ones((t.shape[0], 1)) * ep, x, u),
                                   axis=1)
                # X = np.concatenate((np.ones((t.shape[0], 1)) * ep, x), axis=1)
        ...
        if self.normalize:
            ...
            # Save the normalizing parameters
            norm_coef = np.array([max_state_1, max_state_2, max_input])
            # norm_coef = np.array([max_state_1, max_state_2])
    ...
    #     return np.array(
    #         [[x[1]],
    #          [(-self.c * x[1] - self.k * x[0] - self.k2 * x[0]**3) / self.m]])


class PreprocessMassSpringDamper:
    ...
    def preprocess(self, path: str):
        ...
        Q = utilities.Q_gen(n_states=n_states,
                            add_noise=[0, 1],
                            noise=self.noise)
        # Ad, Bd, Qd = utilities.cont_to_disc(A, B, L, Q, self.dt)
        ...
        for tag, n_eps in n_eps.items():
            for ep in range(n_eps):
                ...
                for i in range(t.shape[0] - 1):
                    x[i +
                      1, :] = x[i, :] + self.dt * (A @ x[i, :] + B @ u[i, :])
                    # x[i + 1, :] = x[i, :] + self.dt * (A @ x[i, :])
        ...
        if self.noise != 0:
            ...
            path = "build/preprocessed_data/msd/variance_{}_snr.bin".format(
                self.noise)
        ...
        if self.normalize:  # Normalize is crap rn, need to fix
            max_state_1 = np.max(
                np.abs(
                    np.concatenate((pykoop_dict['X_train'][:, 1],
                                    pykoop_dict['X_valid'][:, 1]),
            ...
            norm_coef = np.array([max_state_1, max_state_2, max_input])
            path = "build/preprocessed_data/msd/variance_{}_norm_params.bin".format(
                self.noise)
            os.makedirs(os.path.dirname(path), exist_ok=True)
            with open(path, "wb") as f:
                pickle.dump(norm_coef, f)
        ...
        # u_valid = pykoop_dict['X_valid'][:, 0]

        self.pykoop_dict = dict(
            X_train=pykoop_dict['X_train'],
            X_valid=pykoop_dict['X_valid'],
            n_inputs=self.n_inputs,
            x0_valid=x0_valid,
            u_valid=u_valid,
        )
        self.pykoop_dict_true = dict(
            X_train=pykoop_dict_true['X_train'],
            X_valid=pykoop_dict_true['X_valid'],
            n_inputs=self.n_inputs,
            x0_valid=x0_valid,
            u_valid=u_valid,
        ...
        path = 'build/preprocessed_data/msd/'
        os.makedirs(os.path.dirname(path), exist_ok=True)
        with open(path + "variance_{}.bin".format(self.noise), "wb") as f:
            pickle.dump(self, f)
