    ...
    def plot(self, **kwargs):
        ...
        for regressor in self.regressors:
            with open(
                ...
                kp = pickle.load(f)
            if regressor == "TEDMD-AS" or regressor == "FBEDMD-AS" or regressor == "EDMD-AS":
...
class PlotFrobErr():
    ...
    def plot(self, **kwargs):
        ...
        frob_error_B = {}
        koop_matrices_true = {}
        koop_matrices_true_sv = {}
        snr = np.zeros(len(self.variance_lvl))

        for regressor in self.regressors:
            if regressor == "TEDMD-AS" or regressor == "TEDMD" or regressor == "EDMD" or regressor == "FBEDMD":
                continue
            with open(
                    "build/pykoop_objects/{}/variance_{}/kp_{}_{}_{}.bin".
        ...
        for var in self.variance_lvl:
            for regressor in self.regressors:
                if regressor == "EDMD" or regressor == "FBEDMD" or regressor == "TEDMD":
                    continue
                if regressor == "TEDMD-AS" or regressor == "TEDMD":
                    ...
                    with open(
                            "build/pykoop_objects/{}/true_kp/var_{}.bin".
                            format(self.robot, var), "rb") as f:
                        kp_true = pickle.load(f)
                    koop_matrix_true = kp_true.regressor_.coef_.T

                    frob_error_U[regressor] = np.append(
                        frob_error_U[regressor],
                        scipy.linalg.norm(koop_matrix -
                                          koop_matrix_true, 'fro') /
                        scipy.linalg.norm(koop_matrix_true, 'fro')
                    ) if k > 0 else scipy.linalg.norm(
                        koop_matrix - koop_matrix_true,
                        'fro') / scipy.linalg.norm(koop_matrix_true, 'fro')

                    frob_error_A[regressor] = np.append(
                        frob_error_A[regressor],
                        scipy.linalg.norm(
                            koop_matrix[:, :koop_matrix.shape[0]] -
                            koop_matrix_true[:, :koop_matrix_true.shape[0]],
                            'fro') /
                        scipy.linalg.norm(
                            koop_matrix_true[:, :koop_matrix_true.shape[0]],
                            'fro')
                        ...
                        koop_matrix[:, :koop_matrix.shape[0]] -
                        koop_matrix_true[:, :koop_matrix_true.shape[0]], 'fro'
                    ) / scipy.linalg.norm(
                        koop_matrix_true[:, :koop_matrix_true.shape[0]], 'fro')
                    ...
                    frob_error_A[regressor] = np.append(
                        ...
                        scipy.linalg.norm(
                            koop_matrices_true[regressor]
                            [:, :koop_matrices_true[regressor].shape[0]],
                            'fro')) if k > 0 else scipy.linalg.norm(
                                koop_matrices_true[regressor]
                                [:, :koop_matrices_true[regressor].shape[0]] -
                                koop_matrix[:, :koop_matrix.shape[0]],
                                'fro') / scipy.linalg.norm(
                                    koop_matrices_true[regressor]
                                    [:, :koop_matrices_true[regressor].
                                     shape[0]], 'fro')
                    ...
                    frob_error_B[regressor] = np.append(
                        ...
                        scipy.linalg.norm(
                            koop_matrices_true[regressor]
                            [:,
                             koop_matrices_true[regressor].shape[0]:], 'fro')
                    ) if k > 0 else scipy.linalg.norm(
                        koop_matrices_true[regressor]
                        [:, koop_matrices_true[regressor].shape[0]:] -
                        koop_matrix[:, koop_matrix.shape[0]:],
